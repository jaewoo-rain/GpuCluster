...
사용 가능한 환경변수 (모두 FGPU_ 접두사):
  ...
  FGPU_WORKSPACE_ROOT      기본: <repo>/data/sessions (Stage 10 jupyter mode).
                           각 세션의 노트북이 <root>/<session_id>/ 에 저장됨.
  FGPU_PUBLIC_HOST         기본 'localhost'. UI 에 표시되는 jupyter URL 의
                           호스트명. 외부 접속이면 호스트 IP/도메인으로.
...
class Settings(BaseSettings):
    ...
    workspace_root: str = "" # 빈 값이면 <repo>/data/sessions 자동 설정 (Stage 10)
...
def get_settings() -> Settings:
    ...
    if not s.workspace_root:
        s.workspace_root = os.path.join(_default_repo_root(), "data", "sessions")
    return s
