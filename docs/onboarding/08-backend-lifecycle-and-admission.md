...
- [sessions.py:62](../../backend/app/api/sessions.py#L62)~[sessions.py:95](../../backend/app/api/sessions.py#L95)
...
```
- `create()`: [session_manager.py:135](../../backend/app/services/session_manager.py#L135)
- 락 필드 `_create_lock` 정의(주석에 이유까지): [session_manager.py:81](../../backend/app/services/session_manager.py#L81)~[session_manager.py:85](../../backend/app/services/session_manager.py#L85)
- 실제 작업 `_create_locked()`: [session_manager.py:156](../../backend/app/services/session_manager.py#L156)

`_create_locked` 안을 순서대로 보면:
1. 어드미션 검사 (force=False 일 때만): [session_manager.py:168](../../backend/app/services/session_manager.py#L168)~[session_manager.py:171](../../backend/app/services/session_manager.py#L171)
2. 컨테이너 spawn (`to_thread`): [session_manager.py:211](../../backend/app/services/session_manager.py#L211)
3. `Session` 레코드 조립: [session_manager.py:253](../../backend/app/services/session_manager.py#L253)
4. SQLite insert (`to_thread`): [session_manager.py:271](../../backend/app/services/session_manager.py#L271)
...
코드에서 각 조각의 위치:
- 환경변수(`FGPU_RATIO`, `LD_PRELOAD`) 조립: [docker_manager.py:92](../../backend/app/services/docker_manager.py#L92)~[docker_manager.py:95](../../backend/app/services/docker_manager.py#L95)
- 절대 쿼터(`FGPU_QUOTA_BYTES`)와 throttle(`FGPU_COMPUTE_RATIO`) 옵션: [docker_manager.py:96](../../backend/app/services/docker_manager.py#L96)~[docker_manager.py:101](../../backend/app/services/docker_manager.py#L101)
- **환경변수 화이트리스트 전달**: [docker_manager.py:106](../../backend/app/services/docker_manager.py#L106)~[docker_manager.py:109](../../backend/app/services/docker_manager.py#L109)
- `--gpus all` vs `--gpus device=N` (`gpu_index`): [docker_manager.py:121](../../backend/app/services/docker_manager.py#L121)~[docker_manager.py:127](../../backend/app/services/docker_manager.py#L127)
- 훅 `.so` read-only bind mount: [docker_manager.py:130](../../backend/app/services/docker_manager.py#L130)~[docker_manager.py:135](../../backend/app/services/docker_manager.py#L135)
- 실제 실행 `client.containers.run(**run_kwargs)`: [docker_manager.py:157](../../backend/app/services/docker_manager.py#L157)
...
- jupyter 명령 조립: [docker_manager.py:49](../../backend/app/services/docker_manager.py#L49)
- 워크스페이스 마운트: [docker_manager.py:138](../../backend/app/services/docker_manager.py#L138)~[docker_manager.py:142](../../backend/app/services/docker_manager.py#L142)
- 할당된 호스트 포트 읽기(`get_host_port`): [docker_manager.py:165](../../backend/app/services/docker_manager.py#L165)
- 매니저 쪽에서 백오프로 포트 폴링: [session_manager.py:240](../../backend/app/services/session_manager.py#L240)~[session_manager.py:251](../../backend/app/services/session_manager.py#L251)
...
- `remove=False` ([docker_manager.py:149](../../backend/app/services/docker_manager.py#L149)) 라 컨테이너가 종료돼도 자동 삭제되지 않습니다. 종료 후에도 로그를 조회하려는 의도예요. 대신 `DELETE` 를 호출하기 전까지 컨테이너 껍데기가 남습니다.
...
```
- `get()` 전체: [session_manager.py:288](../../backend/app/services/session_manager.py#L288)~[session_manager.py:312](../../backend/app/services/session_manager.py#L312)
- 컨테이너가 데몬에서 사라진 경우(`NotFound`)는 레코드를 지우지 않고 status 만 `"removed"` 로: [session_manager.py:297](../../backend/app/services/session_manager.py#L297)~[session_manager.py:304](../../backend/app/services/session_manager.py#L304)

`list_all()` 은 이 `get()` 을 모든 세션에 대해 `asyncio.gather` 로 **동시에** 돌립니다 ([session_manager.py:314](../../backend/app/services/session_manager.py#L314)~[session_manager.py:320](../../backend/app/services/session_manager.py#L320)). 세션이 10개면 10번 순차로 도커에 묻는 대신 한꺼번에 물어서 응답이 빨라집니다.
...
- 락으로 감싸는 부분: [session_manager.py:135](../../backend/app/services/session_manager.py#L135)~[session_manager.py:154](../../backend/app/services/session_manager.py#L154)
...
- 409 번역: [sessions.py:79](../../backend/app/api/sessions.py#L79)~[sessions.py:92](../../backend/app/api/sessions.py#L92)
- `force=true` 면 검사를 아예 건너뜁니다: [session_manager.py:169](../../backend/app/services/session_manager.py#L169) (oversubscription 데모용)
...
**백엔드는 훅에게 쿼터를 오직 환경변수로만 전달합니다.** `create_container` 가 `FGPU_RATIO`(또는 `FGPU_QUOTA_BYTES`)를 컨테이너 env 로 넣고([docker_manager.py:92](../../backend/app/services/docker_manager.py#L92)~[docker_manager.py:101](../../backend/app/services/docker_manager.py#L101)), 컨테이너 안에서 `LD_PRELOAD` 로 뜬 `libfgpu.so` 가 그 env 를 읽어 자기 쿼터를 정합니다. 그게 전부예요.
