...
### Stage 10 — interactive (Jupyter Lab) sessions

`SessionCreate` accepts `mode: "batch" | "jupyter"` (default `batch` — backwards compatible with Stages 1-9). With `mode="jupyter"`:

- `DockerManager.create_container` ignores user `command`, runs `jupyter lab --ip=0.0.0.0 --port=8888 --no-browser --allow-root --ServerApp.token=<token> --ServerApp.root_dir=/workspace`.
- A random URL-safe token is generated by `secrets.token_urlsafe(24)` per session and stored alongside the record.
- Container's `8888/tcp` is published to a host ephemeral port (docker assigns). `SessionManager` polls `container.attrs['NetworkSettings']['Ports']` after `containers.run` to read the assigned port and persist it as `host_port`.
- A host directory `<workspace_root>/<session_id>/` (default `<repo>/data/sessions/<id>/`) is bind-mounted to `/workspace` so notebooks survive container deletion.
- LD_PRELOAD is preserved — the Jupyter kernel inherits the env, so `torch.empty(...)` in a notebook cell hits the hook normally.
- `DELETE /sessions/{id}?purge_workspace=true` also deletes the workspace dir; default keeps it.

Stage 10 image change: `runtime-image-pytorch/Dockerfile` now `pip install jupyterlab ipywidgets` and `WORKDIR /workspace`. Rebuild required after pulling Stage 10. Stage 1-9 batch behavior is unchanged.

UI: a `mode` radio toggles between batch/jupyter. Jupyter sessions show an "open ↗" button in a new `jupyter` table column; the URL is reassembled client-side from `location.hostname + host_port + token` so it works whether the user is on localhost or hitting the box from another machine.

Verification: `scripts/eval/run_jupyter.sh` creates a jupyter session, polls `http://localhost:<host_port>/api/status` for HTTP 200, writes a host file into the workspace, and confirms the container sees it via `docker exec ls /workspace` and Jupyter's own `/api/contents/host_touched.txt` returns 200. Artifacts under `experiments/jupyter_<TS>/`.

### Stage 10 success criteria

`scripts/eval/run_jupyter.sh` produces `summary.txt` ending in `VERDICT: PASS`. The PASS conditions:

- `Jupyter /api/status` returns 200 within ~15 s of session start (server boots and authenticates the token).
- The host-written file `host_touched.txt` is visible in the container's `/workspace` listing (bind-mount works).
- Jupyter's own `/api/contents/host_touched.txt` returns 200 (Jupyter sees the file at the API level — confirms `--ServerApp.root_dir=/workspace` is correct).

What it proves
- Interactive coding sessions work end-to-end. Notebook cells in the browser run on the host's GPU, with hook quota enforcement intact.
- Workspace persistence — deleting the container without `purge_workspace=true` keeps the user's notebooks on the host.

What it does NOT prove
- Concurrency under heavy multi-user load. Single Jupyter, single user, single notebook tested.
- Idle-session reclamation (a user can leave a notebook open holding GPU memory forever — no auto-stop yet).
- Reverse-proxying Jupyter through the FastAPI backend (port is publicly exposed on the host; firewall is the only gate beyond the Jupyter token).
- Per-user RBAC. Anyone with the URL + token has full kernel access — that's by design for the prototype.

### Stage 11 — admission control (sum-of-ratios ≤ 1.0)

Adds the missing scheduler-layer gate that Backend.AI puts in front of the per-container hook. Without admission, two sessions of `ratio=0.7` and `ratio=0.4` both succeed because the hook only enforces *per-container*; the prototype now refuses the second request unless `force=true`.

- `app/services/admission.py` — pure-function policy module. `gpu_overlaps()`, `sum_used_ratio()`, `check()` (raises `AdmissionDenied`), `usage_snapshot()`. Stdlib only, no docker / no I/O.
- `app/services/session_manager.py` — `create()` now wraps `_create_locked()` under an `asyncio.Lock` so check + spawn is atomic; concurrent POSTs don't both pass the same capacity. `force=True` skips the check. `admission_snapshot()` exposes the per-GPU usage (drives the UI capacity line).
- `app/api/sessions.py` — `AdmissionDenied` → HTTP 409 with structured detail (`requested_ratio`, `currently_used`, `available`, `gpu_index`, `active_sessions`). New `GET /sessions/admission` returns `{"by_gpu": {"all"|"0"|...: {ratio_used, ratio_available, active_sessions}}}`.
- `app/schemas/session.py` — `SessionCreate.force: bool = False`.
- `app/static/index.html` — capacity line (`capacity: GPU all: 0.700/1.000 (1 session)`) above the create form, polled every 3 s alongside sessions; force checkbox; 409 response is parsed and rendered as a friendly hint instead of raw JSON.
- `backend/tests/test_admission.py` — 17 tests covering `gpu_overlaps`, status filtering, GPU isolation, FP tolerance at exact 1.0, force-equivalent (we test the pure function so force just means "don't call check"), and snapshot grouping. No docker required.

GPU-overlap rules
- `gpu_index=None` (= "all GPUs") overlaps with **every** other session. Implication on multi-GPU host: a `None` session counts toward every device's quota; a device-pinned session counts only toward its device.
- Same `gpu_index` integers overlap; different integers are isolated.
- Single-GPU host (RTX 4070): all sessions overlap, so the policy reduces to `sum(active_ratios) ≤ 1.0`.

What this layer does NOT enforce
- Driver-level OOM. If two cooperating tenants pass admission (sum ≤ 1.0) but pile their actual allocations close to the cap, NVIDIA driver may still return `cudaErrorMemoryAllocation` because of CUDA context overhead (~700 MiB per process). Hook's per-container quota also doesn't account for that overhead — by design, since paper measures *quota arithmetic*, not absolute physical bound.
- Time-based admission (sessions decay / expire). Stale `running` records that the docker daemon already lost will be reconciled on next `list_all()`, so they don't permanently jam capacity, but a never-stopping notebook does.
- Cross-host coordination (Stage 9 full).

### Stage 11 success criteria

`pytest backend/tests/test_admission.py` passes 17/17 (no docker / no GPU).

End-to-end via the running backend:
- `GET /sessions/admission` returns `{"by_gpu": {}}` when no sessions, or `{"all": {"ratio_used": 0.7, ...}}` after a 0.7 session is created.
- `POST /sessions {"ratio": 0.4}` after a 0.7 session exists returns **HTTP 409** with detail `{"error": "admission_denied", "currently_used": 0.7, "available": 0.3, ...}`.
- `POST /sessions {"ratio": 0.4, "force": true}` succeeds (oversubscription explicit). The capacity line in the UI flips to `⚠ oversubscribed`.
- After `DELETE /sessions/<0.7-session>`, capacity returns to `0.000/1.000` and a new 0.4 admit passes without `force`.

Concurrency: two simultaneous POSTs of `ratio=0.6` each — exactly one returns 201, the other returns 409 (the `asyncio.Lock` serializes the check-then-insert window). This is the property a global admission gate must hold; without the lock, both could pass the check independently and oversubscribe.

What this proves
- Per-container hook (memory cap) and admission (capacity gate) are **two distinct enforcement layers**. Paper figure: a request flow diagram with the two layers labeled, and a table of which class of failure each catches (admission catches *would-be over-allocation at spawn time*; hook catches *actual per-container over-allocation at runtime*).
